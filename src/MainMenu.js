...
exports = Class(ui.ImageView, function (supr) {
	this.init = function (opts) {
		opts = merge(opts, {
			x: 0,
			y: 0,
			image: "resources/images/FillerMidwayTop.png"
		});
	...
	this.build = function() {
		 ...
		 * the top-level application file.
		 */
		quitbutton.on('InputSelect', bind(this, function () {
			this.emit('titlescreen:quit');
		}));
		 ...
		 * the top-level application file.
		 */
		scorebutton.on('InputSelect', bind(this, function () {
			this.emit('titlescreen:score');
		}));
	};
});
